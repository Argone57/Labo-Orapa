(function(global){
  ...
  function simulateBeam(options){
    ...
    while(true){if(++guard>400){absorbed='loop';break;}let best={...boundaryIntersection(position,direction,width,height),kind:'boundary'};const gravityCandidates=[];
      for(const piece of placed){if(piece.id===passThroughPieceId)continue;const definition=definitionFor(piece.type)||{},edges=edgesFor(piece),directHits=[];edges.forEach(([a,b],edgeIndex)=>{if(piece.id===skipPieceId&&edgeIndex===skipEdgeIndex)return;const hit=intersectRaySegment(position,direction,a,b);if(hit&&definition.isBlackHole)directHits.push(hit);if(hit&&(hit.t<best.t-EPS||(best.kind==='gravity'&&Math.abs(hit.t-best.t)<=EPS)))best={...hit,kind:'edge',piece,edgeType:edgeKind(a,b),edgeIndex};});
        if(definition.isBlackHole&&!refracted&&!directHits.length){const center=piece.center;let gravity=null;if(direction.dx!==0&&Math.abs(Math.abs(position.y-center.y)-1)<EPS){const x=center.x+direction.dx,t=(x-position.x)/direction.dx;if(t>EPS)gravity={t,point:{x,y:position.y}};}else if(direction.dy!==0&&Math.abs(Math.abs(position.x-center.x)-1)<EPS){const y=center.y+direction.dy,t=(y-position.y)/direction.dy;if(t>EPS)gravity={t,point:{x:position.x,y}};}if(gravity){const before={x:gravity.point.x-direction.dx*EPS*100,y:gravity.point.y-direction.dy*EPS*100},cellKey=`${piece.id}:${Math.floor(before.x+EPS)}:${Math.floor(before.y+EPS)}`,candidate={...gravity,kind:'gravity',piece,cellKey};gravityCandidates.push(candidate);if(!cancelledGravityCells.has(cellKey)&&gravity.t<best.t-EPS)best=candidate;}}
      }
